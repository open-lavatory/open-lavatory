...
- Session: transports are negotiated — the first host preference both peers support wins, no common transport fails the session; new `info` option on `createSession`/`connectSession`, `getPeerInfo()`, and a `peer_info` event; empty transport lists and info a receiver would reject now throw at creation
