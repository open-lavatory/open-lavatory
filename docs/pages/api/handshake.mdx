...
type SignalMessageCapabilities = {
  ...
  payload: {
    ...
    info?: {
      ...
      icon?: string;      // base64 data URI — keep small, see below
      url?: string;
    };
  };
  timestamp: number;
...
Some signaling protocols enforce small message limits (ntfy in particular), and the whole packet must survive base64 + encryption overhead.
Icons should therefore be tiny (a few KB at most — think 64×64 PNG) or omitted in favor of `url`, from which a peer can fetch richer metadata after the transport is up.
...
An empty intersection puts signaling in the `ERROR` state with a "no common transport" reason.
