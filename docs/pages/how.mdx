...
The exact packets and their order are documented in the [handshake protocol](/api/handshake) page.
...
You can read more about the specifics in the [api documentation](/api/intro).

## Signaling

To establish a connection both devices connect to (public) infrastructure running a signaling protocol.
...
Currently the transport protocol supported is [WebRTC](/api/transport/webrtc).

| Protocol | Description |
|----------|-------------|
| [WebRTC](/api/transport/webrtc) | A p2p messaging protocol for sending and receiving messages between devices |

Ideally at some point support for direct transport layer communication through [WebHID](), [WebUSB](), [WebSerial]() and [WebBluetooth]().
