...
import { OpenLVGlobals, connectSession, SessionStatus } from '@openlv/react-native'; // [!code ++]
...
export default function App() {
  ...
  const startSession = async () => {
    ...
    await session.status.until((status) => status === SessionStatus.CONNECTED);
    setConnected(true);
  }
...
These polyfills override certain global objects (e.g., `window.crypto`) and install once for the app process. Keep `OpenLVGlobals` mounted near the application root.
